...
describe('jsonSchemaToZodRuntime', () => {
  ...
  describe('when the field drives specialized form rendering', () => {
    it('keeps a date-time string a date field rather than a plain text field', () => {
      const shape = (jsonSchemaToZodRuntime(schemas['a date-time string'] as JSONSchema7) as z.ZodObject<any>).shape;
    ...
    it('keeps a dictionary a record field, so its entries are not discarded', () => {
      const shape = (jsonSchemaToZodRuntime(schemas['a dictionary of strings'] as JSONSchema7) as z.ZodObject<any>)
        .shape;

      expect(inferFieldType(getBaseSchema(shape.m))).toBe('record');
      expect(
        jsonSchemaToZodRuntime(schemas['a dictionary of strings'] as JSONSchema7).parse({ m: { k: 'v' } }),
      ).toEqual({
        m: { k: 'v' },
      });
    });
    ...
    it('keeps an anyOf a union rather than an intersection', () => {
      const shape = (jsonSchemaToZodRuntime(schemas['an anyOf union'] as JSONSchema7) as z.ZodObject<any>).shape;
