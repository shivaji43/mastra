...
- Fix the `token` chunking strategy ignoring `stripWhitespace: false`. `TokenTransformer.fromTikToken()` now forwards `stripWhitespace`, `addStartIndex`, and `separatorPosition` to the transformer instead of silently dropping them, so chunks produced with `overlap: 0` and `stripWhitespace: false` rejoin to the original text. ([#24222](https://github.com/mastra-ai/mastra/pull/24222))

- Fixed RAG document chunking so chunks no longer exceed maxSize when the configured overlap is large relative to maxSize. Previously a large overlap could carry over enough content to push the next chunk past the limit. ([#24177](https://github.com/mastra-ai/mastra/pull/24177))
