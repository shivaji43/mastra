...
- Fixed dataset storage to preserve authored strings and distinguish omitted item fields from explicit null through updates and version history. Cleared dataset schemas now read as undefined. Previously lost values cannot be recovered automatically. ([#23937](https://github.com/mastra-ai/mastra/pull/23937))
...
- Implemented `updateNotificationsStatus` with a single `updateMany` so marking many notifications seen no longer issues one write per record. ([#23718](https://github.com/mastra-ai/mastra/pull/23718))
