...
> **Note:** Observational Memory `scope: 'resource'`, used in this lesson, is now deprecated. For cross-thread continuity, keep the default thread scope and enable [retrieval mode](/docs/memory/observational-memory#retrieval-mode) or [resource-scoped working memory](/docs/memory/working-memory#resource-scoped-memory-default).
