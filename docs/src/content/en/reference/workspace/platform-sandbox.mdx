...
description: "Use PlatformSandbox to provision, inspect, execute commands in, and delete isolated sandboxes within a Mastra Platform environment."
packages:
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    description:
      'Default directory for command execution when no per-command `cwd` is given. A per-command `cwd` always wins. Use an absolute path.',
    isOptional: true,
  },
  {
    name: 'timeout',
