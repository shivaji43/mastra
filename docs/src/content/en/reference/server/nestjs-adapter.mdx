...
description: "The @mastra/nestjs package provides a NestJS module for running Mastra with the Express-based NestJS platform."
packages:
  - "@mastra/nestjs"
---
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Per-route auth overrides. Keys are `METHOD:PATH`. Custom routes from `server.apiRoutes` (`registerApiRoute()`) are added automatically based on their `requiresAuth` setting.',
    isOptional: true,
  },
  {
    name: 'tools',
