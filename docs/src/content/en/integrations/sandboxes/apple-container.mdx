...
description: "Run Mastra workspace commands in local OCI Linux containers with Apple container, including environment variables, mounts, networking, and reconnection."
packages:
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    description:
      'Working directory inside the container, used as the default for command execution when no per-command `cwd` is given. A per-command `cwd` always wins.',
    isOptional: true,
...
When `readonlyRootfs` is enabled, make sure `workingDirectory` points to a path supplied by the image or a bind mount. A writable tmpfs is also supported.
