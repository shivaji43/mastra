...
```mermaid
flowchart LR
  accTitle: Semantic recall before and after an LLM request
  accDescr: The pre-LLM pipeline embeds a user prompt and queries the vector database. The results become the semantic recall part of the context messages, which feed the context window. The post-LLM pipeline embeds all new messages and inserts them into the vector database.

  subgraph pre["RAG pipeline pre-LLM"]
    direction TB
    prompt(( user prompt )) --> queryEmbedding([create embeddings<br/>from user prompt])
    queryEmbedding --> query@{ shape: cyl, label: "query vector DB" }
  end

  subgraph context["context"]
    direction BT
    contextWindow([context window])
    messages[context messages<br/><br/>system instruction<br/>working memory<br/><strong>semantic recall</strong><br/>conversation history<br/>new user message]
    messages --> contextWindow
  end

  subgraph post["RAG pipeline post-LLM"]
    direction TB
    llm([LLM request]) --> messageEmbedding([create embeddings<br/>from all new messages])
    messageEmbedding --> insert@{ shape: cyl, label: "insert in<br/>vector DB" }
  end
