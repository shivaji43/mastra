...
<Callout type="warning">
  `'continue'` has no backstop for a sustained outage. Unobserved messages stay pending and keep accruing in the main
  agent's context, so a long outage moves the failure from the memory layer to the model's context limit.
</Callout>
...
`scope: 'resource'` is deprecated and will be removed in a future release. Mastra logs a warning the first time Observational Memory is created with resource scope. Remove the `scope` option to use the default thread scope, and use the [alternatives below](#cross-thread-continuity-without-resource-scope) for cross-conversation continuity.
...
- [Resource-scoped working memory](/docs/memory/working-memory#resource-scoped-memory-default): Stores small, durable facts about the user that every thread can read.
