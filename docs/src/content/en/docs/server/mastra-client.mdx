...
```typescript title="lib/stored-workflow.ts"
import { MastraClient } from '@mastra/client-js'
...
Durable storage requires a configured storage adapter that supports the `workflowDefinitions` domain. Without that domain, Core can register a workflow in memory, but the server's stored-workflow API can't preserve it across restarts.
