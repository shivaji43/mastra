...
description: "Run Mastra evals against memory-enabled agents using shared or per-item threads, observational memory, runEvals, and dataset experiments."
packages:
  - "@mastra/core"
  - "@mastra/memory"
---
...
`dataset.startExperiment()` accepts `targetType` and `targetId` for a registered target, or an inline `task` function. For a registered memory-enabled agent, the runner creates a fresh thread per item by default. See [memory-enabled experiment targets](/docs/evals/experiments#memory-enabled-agents) for resource isolation and request context options.
