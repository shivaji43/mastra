...
For custom execution, pass an inline `task` function instead. See [dataset experiments with an inline task](/docs/evals/evals-with-memory#dataset-experiments-with-an-inline-task) for an example that controls each item's memory thread.
...
Each item's `input` is passed directly to the agent. Use a supported message format, such as a prompt string, a message object, or an array of messages. See the [`agent.generate()` reference](/reference/agents/generate) for accepted inputs.
