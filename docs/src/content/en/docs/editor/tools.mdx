...
1. Register the provider in your Editor configuration:
   ...
   Composio tool slugs use a format like `GITHUB_CREATE_ISSUE`. By default, tool calls use the connection pinned by the agent's author. To route calls to each end user's own account instead, see [connection scope](https://agent-builder.mastra.ai/tool-providers#connection-scope) in the Agent Builder docs.
