...
Everything you can do in Studio is also available programmatically through [`mastra.getEditor()`](/reference/core/getEditor). This is useful for scripting bulk updates or seeding stored configurations from code, or alternatively building automation that tunes agents based on evaluation results.
...
By default, [`mastra.getAgentById()`](/reference/core/getAgentById) loads the published (active) version of the stored override.
