---
title: "Workflows"
description: "Build deterministic Mastra workflows with typed steps, agents, tools, branching, parallel execution, loops, suspension, state, and Studio visualization."
packages:
  - "@mastra/core"
---
...
Workflows let you define complex task sequences with clear, structured steps instead of relying on one agent to reason through the entire process. You control the task breakdown along with the flow and timing of data. Workflows use the built-in execution engine by default, while managed infrastructure is available through [workflow runners](/docs/deployment/workflow-runners) such as Inngest.
...
<Tabs>
  <TabItem value="zod" label="Zod">
    ```typescript {9,12,15,16} title="src/mastra/workflows/test-workflow.ts" oxfmt:false
  ...
  <TabItem value="valibot" label="Valibot">
    ```typescript {10,13,16,17} title="src/mastra/workflows/test-workflow.ts" oxfmt:false
    import { createWorkflow, createStep } from "@mastra/core/workflows";
    import * as v from "valibot";
    import { toStandardJsonSchema } from "@valibot/to-json-schema";
...
```typescript {26} title="src/mastra/workflows/test-workflow.ts" oxfmt:false
const step1 = createStep({...});
...
```typescript oxfmt:false {6} title="src/mastra/workflows/test-workflow.ts"
