...
```mermaid
flowchart LR
  accTitle: Composing an agent as a workflow step
  accDescr: The workflow starts, runs step1 as an agent, passes its output to step2, which calls a tool, and then ends when the workflow is committed.
  ...
  start(( start )) -- ".then(step1)<br/>in" --> step1
  step1 -- ".then(step2)<br/>out" --> step2([step2])
  step2 -- ".commit()<br/>out" --> stop(( end ))
  step2 <--> tool([tool])
```
...
```mermaid
flowchart LR
  accTitle: Composing a tool as a workflow step
  accDescr: The workflow starts with step1, which calls an agent, then passes its output to step2 as a tool before ending when the workflow is committed.
  ...
  start(( start )) -- ".then(step1)<br/>in" --> step1([step1])
  step1 -- ".then(step2)<br/>out" --> step2
  step2 -- ".commit()<br/>out" --> stop(( end ))
  step1 <--> agent([agent])
```
