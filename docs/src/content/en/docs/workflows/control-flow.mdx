...
description: "Control Mastra workflow execution with sequential steps, parallel branches, conditional logic, loops, nested workflows, mapping, and output merging."
packages:
  - "@mastra/core"
---
...
```mermaid
flowchart LR
  accTitle: Choosing a workflow step with conditional branching
  accDescr: The workflow runs step1, then follows either branch A or branch B before committing and ending.

  start(( start )) -- ".then(step1)" --> step1([step1])
  step1 -- out --> branchCall@{ shape: text, label: ".branch([A, B])" }
  branchCall --> stepA([A])
  branchCall accent1@--> stepB([B])
  stepB accent2@-- "out<br/>.commit()" --> stop(( end ))

  class stepB,stop accent
```

```typescript {30-33} title="src/mastra/workflows/test-workflow.ts" oxfmt:false
const step1 = createStep({...})
...
```typescript oxfmt:false {14} title="src/mastra/workflows/test-workflow.ts"
const step1 = createStep({...});
...
```typescript oxfmt:false {15} title="src/mastra/workflows/test-workflow.ts"
const step1 = createStep({
...
```typescript oxfmt:false title="src/mastra/workflows/test-workflow.ts"
const step1 = createStep({...})
